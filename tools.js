...
const { MemoryCookieStore, parse, CookieJar, Cookie } = require('tough-cookie');
const { getPage } = require('./puppeteerHelper');
...
class Tools {
	...
	getLocalCookie = async (noValidating = false) => {
		...
		try {
			...
			if (!rawcookie.startsWith('{') && !rawcookie.endsWith('}')) {
				...
				this.reqTools.cookiejar = new CookieJar(store);
			} else {
				let resolve = null;
				let reject = null;
			...
			if (noValidating) {
				this.isLogin = true;
			} else if (await this.validateCookies()) {
				this.isLogin = true;
				// return true
			}
			return this.isLogin;
	...
	saveJson = async cookies => {
		const cookiePath = path.join(__dirname, 'cookie.json');
		let json;
		if (cookies) {
			json = cookies;
		} else {
			json = this.reqTools.cookiejar.toJSON();
		}
	...
	getTakIdHandle = async skuId => {
		const url = `https://marathon.jd.com/seckill/seckill.action?skuId=${skuId}&num=1&rid=${this.getRandomNumber()}`;
	...
	getOrderData = async (skuId, retry = 20) => {
		while (retry--) {
			try {
				...
				const defaultAddress = initInfo.address;
				const invoiceInfo = initInfo.invoiceInfo || {};
				const token = initInfo.token;
				const takId = await this.takHandle();
				const data = {
					skuId: skuId,
					num: 1,
	...
	submitCartOrder = async () => {
		...
		if (config.pwd) {
			payload['submitOrderParam.isBestCoupon'] = 1;
	...
	checkSkus = async (skus, ids = [], area, uncheck) => {
		...
		while (i--) {
			try {
				...
				const vendors = (result.resultData || {}).cartInfo
					? result.resultData.cartInfo.vendors
					: [];
